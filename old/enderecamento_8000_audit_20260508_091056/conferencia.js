...
function getLocalByBarcode_(storeName, barcode) {
  ...
  // Fonte de verdade: "Localizações produtos"
  const codProduto = getProductCodeByBarcode_(storeName, barcodeStr);
  if (codProduto) {
    produtoEncontrado = true;
    locais = getLocaisByProductCode_(storeName, codProduto);
  }

  // Se não achou pelo catálogo de códigos de barras, tenta usar o próprio valor como cod_produto
  if (!locais || locais.length === 0) {
  ...
  if (!locais || locais.length === 0) {
    return produtoEncontrado ? {localCorreto: '', locaisCompletos: []} : null;
  }

  const localFormatado = formatarLocais(locais);
  return {
    localCorreto: localFormatado,
...
function startAddressing(storeName, barcode) {
  ...
  return {status:'ready', localCorreto: prod.localCorreto, locaisCompletos: prod.locaisCompletos};
}
...
function finalizeAddressing(storeName, barcode, localBipado, operador, quantidade, dataValidade) {
  ...
  const codProduto = getProductCodeByBarcode_(storeName, barcode) || barcode; // Se não encontrar, usa o barcode como fallback

  const produtoNome = (getDescricaoByEan_(storeName, barcode) || {}).descricao || getDescricaoByProductCode_(storeName, codProduto) || '';
  ...
  if (ok) {
    try {
      ...
      const codProdutoParaEstoque = getProductCodeByBarcode_(storeName, barcode);
