...
def _build_dashboard_data(
    ...
    def _merge_product_row(row: dict[str, Any]) -> dict[str, Any]:
        ...
        if product_code and product_code != "Vazio" and product_code not in base_produtos_map:
            empty_row = dict(row)
            empty_row["product_code"] = "Vazio"
            empty_row["product_name"] = ""
            empty_row["quantidade"] = None
            empty_row["curva"] = ""
            empty_row["grupo"] = "Vazio"
            empty_row["grupo_alocado"] = "Vazio"
            empty_row["categoria_armazenagem"] = ""
            empty_row["subcategoria"] = ""
            empty_row["nm_fabricante"] = ""
            empty_row["venda_total"] = None
            empty_row["altura_cm"] = None
            empty_row["peso_kg_unitario"] = None
            empty_row["is_pesado"] = False
            empty_row["is_alto"] = False
            empty_row["is_pequeno"] = False
            empty_row["is_fragil"] = ""
            empty_row["degelo"] = ""
            empty_row["info_hover"] = "<b>Escaninho Vazio</b>"
            empty_row["cor_grupo"] = COR_MAP_GRUPO["vazio"]
            empty_row["_product_code_norm"] = "Vazio"
            return empty_row
        product_info = (
    ...
    for location_id in ordered_locations:
        ...
        for row_index, row in enumerate(location_rows, start=1):
            merged_row = _merge_product_row(row)
...
def _build_content_html(
    ...
    for rua_num in sorted(ruas.keys()):
        ...
        for equip_num in sorted(equipamentos.keys()):
            ...
            equip_grid_html = (
                f'<div class="equipamento-grid" style="--num-cols: {num_cols + 1}; --num-rows: {num_rows + 1};">'
            )
            equip_grid_html += '<div class="grid-cell header-cell corner-cell"></div>'
            for j in range(1, num_cols + 1):
                equip_grid_html += f'<div class="grid-cell header-cell col-header">{ALFABETO[j-1]}</div>'
            ...
            content_html += (
                f'<div class="equipamento" id="{equip_id}">'
                ...
                f'<strong>Equip. #{equip_num}{degelo_emoji}</strong><span>({escape_html(tipo_equip_final)})</span>'
...
def _build_unallocated_section(
    ...
    if final_unallocated_list:
        ...
        for index, falha in enumerate(final_unallocated_list):
            ...
            instance_index = falha.get("escaninho_index") or 1
            ...
            unallocated_products_list.append(
                {
                    "id": product_id,
                    "instance_id": f"unallocated::{falha.get('product_code')}::{instance_index}",
                    ...
                    "dataset": {
                        ...
                        "instanceId": f"unallocated::{falha.get('product_code')}::{instance_index}",
...
def get_initial_data(source: Any) -> dict[str, Any]:
    ...
    dashboard_data = _build_dashboard_data(plano_data, base_produtos_map)
