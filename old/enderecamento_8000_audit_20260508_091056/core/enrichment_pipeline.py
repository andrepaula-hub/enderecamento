...
def run_etl_to_base_products(master_sheet_id: str, mix_sheet_id: str, target_sheet_id: str) -> dict[str, Any]:
    ...
    all_mix_codes = {_norm_code(code) for code in mix_df["product_code"].tolist() if _norm_code(code)}
    zero_qty_codes = {
    ...
    for _, base in mix_df.iterrows():
        ...
        if tipo_equip_base == "geladeira_alta" and is_geladeira_pode:
            escaninhos_necessarios = min(escaninhos_necessarios, 4)
    ...
    if existing_code_idx != -1 and base_values:
        for row in base_values[1:]:
            ...
            if not code:
                continue
            if code not in existing_row_by_code:
                existing_order.append(code)
            existing_row_by_code[code] = padded[: len(target_headers)]

    plano_values = target.read_values(SHEET_PLANO_FINAL)
    ...
    for code in existing_order:
        if code in zero_qty_codes:
            continue
        if code in allocated_codes and code not in output_codes and code in existing_row_by_code and code not in all_mix_codes:
            rows.append(existing_row_by_code[code])
            output_codes.add(code)
