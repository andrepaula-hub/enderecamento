...
def save_batch_moves_gsheet(sheet_id: str, moves: list[dict[str, Any]], user: str = "local") -> dict[str, Any]:
    client = GSheetsClient(sheet_id)
...
def change_equipment_type_gsheet(
    ...
    for item in equip_rows:
        ...
        nivel = str(row_data[nivel_col] if nivel_col >= 0 and nivel_col < len(row_data) else "").strip()
        is_hot_zone = nivel in niveis_hot_zone
        is_nivel_alto = nivel == nivel_alto
        is_nivel_inferior = nivel == nivel_inferior

        if recolher_produtos and produto and produto != "Vazio":
            product_name = str(
                row_data[product_name_col] if product_name_col >= 0 and product_name_col < len(row_data) else ""
            ).strip()
            new_row = [
                _build_new_row_value(header, idx, None, row_data, headers)
                for idx, header in enumerate(headers)
            ]
            logs.append(
                {
        ...
        if tipo_col >= 0:
            new_row[tipo_col] = new_type
        if tipo_final_col >= 0:
            new_row[tipo_final_col] = new_type
        if capacidade_col >= 0:
            new_row[capacidade_col] = capacidade_real
        if hot_zone_col >= 0:
            new_row[hot_zone_col] = is_hot_zone
        if nivel_alto_col >= 0:
            new_row[nivel_alto_col] = is_nivel_alto
        if nivel_inf_col >= 0:
            new_row[nivel_inf_col] = is_nivel_inferior
    ...
    if total_novo < total_atual:
        remover = total_atual - total_novo
        empty_rows = [
            item["row_number"]
            for item in equip_rows
            if not str(item["row"][product_col] if product_col < len(item["row"]) else "").strip()
            or str(item["row"][product_col]).strip() == "Vazio"
        ]
        if not recolher_produtos and len(empty_rows) < remover:
            return {"success": False, "error": f"Não há escaninhos vazios suficientes para reduzir ({remover} necessários)."}
        if recolher_produtos:
            rows_to_delete = [item["row_number"] for item in equip_rows][-remover:]
        else:
            rows_to_delete = empty_rows[-remover:]
        client.delete_rows(SHEET_PLANO_FINAL, rows_to_delete)

    if total_novo > total_atual:
        to_add = total_novo - total_atual
        data_after = client.read_values(SHEET_PLANO_FINAL)
        data_after_rows = data_after[1:] if data_after else []
        existing_locs: set[str] = set()
        for row in data_after_rows:
            row_rua = int(to_float(row[rua_col] if rua_col < len(row) else 0))
            row_equip = int(to_float(row[equip_col] if equip_col < len(row) else 0))
            if row_rua == rua_num and row_equip == equip_num:
                loc_id = str(row[loc_col] if loc_col < len(row) else "").strip()
                if loc_id:
...
def update_base_product_gsheet(
    ...
    *,
    ...
    if target_row_number is not None:
        ...
        return {"success": True, "mode": "update", "product_code": updated_row[code_idx]}

    client.append_rows(SHEET_BASE_PRODUTOS, [updated_row])
    return {"success": True, "mode": "create", "product_code": updated_row[code_idx]}
...
def generate_slots_from_cadastro_gsheet(
    ...
    for row in sorted_cadastro:
        try:
            rua_num = int(float(str(row[cad_rua]).replace(",", ".")))
            equip_num = int(float(str(row[cad_equip]).replace(",", ".")))
        except Exception:
            continue
